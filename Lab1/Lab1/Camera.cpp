...
void Camera::computeMatricesFromInputs()
{
	...
	// Get mouse position
	double xpos, ypos;
	glfwGetCursorPos(initW->GetWindow(), &xpos, &ypos);

	// Reset mouse position for next frame
	glfwSetCursorPos(initW->GetWindow(), 1024/2, 768/2);
	...
	// Move forward
	if (glfwGetKey( initW->GetWindow(), GLFW_KEY_W ) == GLFW_PRESS)
	{
		position += direction * deltaTime * speed;
	}
	// Move backward
	if (glfwGetKey( initW->GetWindow(), GLFW_KEY_S ) == GLFW_PRESS)
	{
		position -= direction * deltaTime * speed;
	}
	// Strafe right
	if (glfwGetKey( initW->GetWindow(), GLFW_KEY_D ) == GLFW_PRESS)
	{
		position += right * deltaTime * speed;
	}
	// Strafe left
	if (glfwGetKey( initW->GetWindow(), GLFW_KEY_A ) == GLFW_PRESS)
	{
		position -= right * deltaTime * speed;
	}
